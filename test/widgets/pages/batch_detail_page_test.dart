...
//
// Same route-argument bug as registration_station_page_test.dart:
// _navigateToRegister here passes {'tableName': ..., 'compositeKeyData':
// ...} but route.dart's dynamicRegister case only looks for a `handler`
// key, so every "ใส่ข้อมูล"/edit action lands on that case's own error
// page instead of the dynamic form.
...
void main() {
  ...
  testWidgets('the fermentation section\'s "ใส่ข้อมูล" button hits the route\'s error page (documented bug)', (tester) async {
    await tester.pumpWidget(wrapPage(
      const BatchDetailPage(batchNo: 'B-001', processingStationNo: 'PS-001'),
    ));
    await tester.pumpAndSettle();
